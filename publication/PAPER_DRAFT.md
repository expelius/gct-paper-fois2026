...
To anchor the cardinality argument empirically, we performed two-direction ablations. The downward direction (`publication/cardinality_ablation.py`) collapses the twelve IVM slots into the four Aristotelian causes (FORMAL ← {SPECIFICATION, INSTANTIATION, CODE}; MATERIAL ← {SUBSTRATE, THERMO}; EFFICIENT ← {CAUSE, PREDECESSOR, SUCCESSOR}; FINAL ← {EFFECT, CONTAINER, PEER_COHERENT, PEER_CONTRADICTORY}), and the §5 metrics were recomputed on the collapsed graph. Three measurable losses surface. The maxim engine drops from twelve slot-specific maxims to four cause-level maxims — a 67% reduction in distinct inference rules, with the §5.2 11/11 violation-detection result no longer reproducible because cause-level rules are too coarse to distinguish the eleven slot-specific violation patterns. Slot-level addressability drops from 8 to 4 distinct values populated on phase-9 (50% reduction; rises to 12 → 4 = 67% on the §6.4 fully-populated biomedical corpus). AC ≥ 0.5 prevalence inflates 1.44× (16.0% → 23.1%) because the collapse merges sub-cause distinctions the 12-slot schema preserved.

The upward direction (`publication/cardinality_expansion.py`) splits each of the twelve IVM slots into 2-3 sub-modalities for an icosahedral 30-sub-slot vocabulary (CAUSE → {DIRECT, MEDIATED, NECESSARY}; PREDECESSOR → {TEMPORAL, LOGICAL, REFINEMENT}; CONTAINER → {PHYSICAL, CONCEPTUAL, DOCUMENTARY}; etc.), and a deterministic heuristic classifier consuming only existing edge metadata (`slot`, `evidence`, source/target id-prefix, scale gap) re-types every edge. Three measurable findings surface. Only **15 of 30** sub-slots receive at least one edge and only **9 of 30** receive ≥10 edges, so 15 of 30 maxims would be vacuous — a 50% sparsity rate, sharply higher than the 33% sparsity of the 12-slot schema. The mean AC drops from 0.284 to 0.044 and the AC ≥ 0.5 mid-class collapses from 16.0% of structural nodes to **0.0%**, eliminating the diagnostic threshold §5.1 uses to flag investigative gaps. Of the fifteen empty sub-slots, **eight are structurally unreachable from current annotation**: rules to populate them would require modal/counterfactual annotation (CAUSE_NECESSARY, EFFECT_DIRECT), fresh extractor coverage of currently-empty parent slots (CODE_*, THERMO_*, SUCCESSOR_*, PEER_CONTRADICTORY_*), or concept-subsumption typing (PEER_COHERENT_GENERALIZATION).

These two ablations form a **sandwich** that anchors the cardinality argument on both sides: the four-slot schema is too coarse (loses 67% of inference rules and the AC discriminative threshold), and the thirty-slot schema is too fine (50% of sub-slot maxims vacuous, AC mid-class collapses to zero). The twelve-slot schema is therefore empirically the **smallest cardinality at which the maxim engine retains its current discriminative power** AND the **largest cardinality at which the corpus avoids sub-slot sparsity**. Honest caveat: the upward expansion is heuristic, not authoritative — it relies on existing metadata, so sub-slots whose distinction is not encoded in current annotation are undercounted, and the under-population *is* the finding that this corpus's knowledge-management practice uses ~12-15 sub-categories. The full upward study requiring fresh sub-slot annotation by an independent reviewer remains future work for the strong claim (*30 is too fine for any plausible knowledge-management corpus*); the present sandwich supports the weaker claim sufficient for §7.3 (*30 is too fine for THIS corpus's current metadata, and 4 is too coarse for any corpus that wants discriminative inference rules*).
