...
Three principal limitations qualify the claims advanced. First, the twelve-slot schema is derived from the combinatorial structure of the cuboctahedron. This is an argument from the theoretical commitments of the research programme in which the GCT was developed, not a derivation from first principles of knowledge representation theory. Whether the cuboctahedron's formal properties constitute a knowledge-representation-theoretic justification or a felicitous structural analogy is a question this paper can only partially anchor. To anchor the cardinality argument empirically we performed two-direction ablations on the phase-9 snapshot. The downward direction (\texttt{cardinality\_ablation.py}) collapses the twelve IVM slots into the four Aristotelian causes (FORMAL $\leftarrow$ \{SPECIFICATION, INSTANTIATION, CODE\}; MATERIAL $\leftarrow$ \{SUBSTRATE, THERMO\}; EFFICIENT $\leftarrow$ \{CAUSE, PREDECESSOR, SUCCESSOR\}; FINAL $\leftarrow$ \{EFFECT, CONTAINER, PEER\_COHERENT, PEER\_CONTRADICTORY\}) and recomputes the \S\ref{sec:evaluation} metrics. Three losses surface: the maxim engine drops from twelve slot-specific maxims to four cause-level maxims (\textbf{67\% reduction}; the 11/11 violation-detection result of \S\ref{sec:maxims} is no longer reproducible because cause-level rules are too coarse); slot-level addressability drops from 8 to 4 distinct values populated on phase-9; and AC $\geq 0.5$ prevalence inflates 1.44$\times$ (16.0\% $\rightarrow$ 23.1\%) because the collapse merges sub-cause distinctions the 12-slot schema preserved. The upward direction (\texttt{cardinality\_expansion.py}) splits each of the twelve IVM slots into 2--3 sub-modalities for a thirty-sub-slot icosahedral vocabulary (CAUSE $\rightarrow$ \{DIRECT, MEDIATED, NECESSARY\}; PREDECESSOR $\rightarrow$ \{TEMPORAL, LOGICAL, REFINEMENT\}; CONTAINER $\rightarrow$ \{PHYSICAL, CONCEPTUAL, DOCUMENTARY\}; etc.) and re-types every phase-9 edge using a deterministic heuristic classifier that consumes only existing edge metadata (slot field, lower-cased evidence string, source/target id-prefix). Three findings surface: only \textbf{15 of 30} sub-slots receive at least one edge, so fifteen maxims would be vacuous (50\% sparsity, sharply higher than the 33\% vacuity of the twelve-slot schema); mean AC drops from 0.284 to 0.044 and AC $\geq 0.5$ prevalence collapses from 16.0\% to \textbf{0.0\%}, eliminating the diagnostic threshold \S\ref{sec:ac} uses to flag investigative gaps; and eight sub-slots are \emph{structurally unreachable from current annotation} (they require modal or counterfactual annotation, fresh extractor coverage of currently-empty parent slots, or concept-subsumption typing). Together these ablations form a \textbf{sandwich}: the four-slot schema is too coarse (67\% inference-rule loss, AC threshold rendered meaningless) and the thirty-slot schema is too fine (50\% vacuous maxims, AC threshold collapses to empty). The twelve-slot schema is empirically the \emph{smallest} cardinality at which the maxim engine retains discriminative power AND the \emph{largest} at which the corpus avoids sub-slot sparsity. Caveat: the upward expansion is heuristic, not authoritative---full upward validation requires fresh sub-slot annotation by an independent reviewer and remains future work for the strong claim that thirty slots are too fine for any plausible knowledge-management corpus; the present sandwich supports the weaker claim sufficient for this limitation: thirty is too fine for \emph{this} corpus's current metadata.
